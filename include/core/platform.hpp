...
inline std::vector<std::string> get_visual_studio_paths() {
  return {"C:\\Program Files\\Microsoft Visual "
          ...
          "Studio\18\\Professional\\Common7\\IDE\\devenv.exe",
          "C:\\Program Files\\Microsoft Visual "
          "Studio\18\\Enterprise\\Common7\\IDE\\devenv.exe",
          "C:\\Program Files\\Microsoft Visual "
...
inline std::vector<std::string> get_doxygen_paths() {
  if constexpr (is_windows) {
    return {"C:\\Program Files\\doxygen\\bin\\doxygen.exe",
    ...
    return {"/usr/bin/doxygen", "/usr/local/bin/doxygen"};
  }
}
 ...
 * @param terminal_emulator Optional specific terminal to use (empty =
 * auto-detect)
...
inline std::string get_default_cmake_generator() {
  if constexpr (is_windows) {
    return "Visual Studio 17 2022";  // Default to VS 2022, with fallback logic
                                     // elsewhere
  } else if constexpr (is_macos) {
