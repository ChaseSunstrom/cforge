...
public:
   ...
   */
  virtual std::string generate_cmake_setup(const test_config::FrameworkConfig &config) const = 0;
   ...
   */
  virtual std::vector<test_result> parse_output(const std::string &output) const = 0;
   ...
   */
  virtual std::vector<std::string> parse_test_list(const std::string &output) const = 0;
};
