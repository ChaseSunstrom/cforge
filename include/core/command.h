 ...
 * @details This structure holds the command line arguments parsed from the input.
 */
typedef struct {
 ...
 * @details This structure holds the context for executing a command, including the command arguments and working directory.
 */
typedef struct {
 ...
 * @details This function will call the appropriate command handler based on the command specified in the arguments.
 */
cforge_int_t cforge_dispatch_command(cforge_context_t* ctx);

/**
 * @brief Set the verbosity level for logging
 * @details This function sets the verbosity level for logging output. It can be set to quiet, normal, or verbose.
 */
