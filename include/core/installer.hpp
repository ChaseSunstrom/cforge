...
public:
     ...
     * @param install_path Path to install the project to. If empty, uses default path.
     * @param add_to_path Whether to add the installation bin directory to PATH
     * @return True if installation was successful
     */
    bool install_project(const std::string& project_path, const std::string& install_path = "", bool add_to_path = false);
...
private:
     ...
     */
    bool copy_files(const std::filesystem::path& source, const std::filesystem::path& dest, 
