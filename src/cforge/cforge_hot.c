...
static int platform_copy_file(const char *src, const char *dst, int max_retries, int delay_ms) {
  int attempt;
...
static void extract_dir_and_base(const char *library_path,
  ...
  for (i = 0; i < len; ++i) {
    if (library_path[i] == '/' || library_path[i] == '\\') {
      last_sep = i + 1;
    }
    if (library_path[i] == '.') {
      dot_pos = i;
...
static void versioned_path(const struct cforge_hot_ctx *ctx, int ver, char *out, size_t out_max) {
#ifdef _WIN32
  snprintf(out, out_max, "%s%s_hot_%03d%s", ctx->lib_dir, ctx->base_name, ver, ctx->lib_ext);
#else
  snprintf(out, out_max, "%s%s_hot_%03d%s", ctx->lib_dir, ctx->base_name, ver, ctx->lib_ext);
#endif
}
 ...
 */
static void cleanup_versioned_copies(const struct cforge_hot_ctx *ctx, int keep_version) {
  int v;
  char path[1024];
...
cforge_hot_ctx *cforge_hot_load(const char *library_path) {
  ...
  if (!ctx->handle) {
    ...
    snprintf(err_buf, sizeof(err_buf), "LoadLibraryA failed (error %lu): %s", code, copy_path);
    set_error(err_buf);
#else
    set_error(dlerror());
#endif
...
int cforge_hot_reload(cforge_hot_ctx *ctx) {
  ...
  if (!ctx) {
    set_error("ctx is NULL");
    return -1;
  }

  signal_val = read_signal_file(ctx->signal_path);
  if (signal_val < 0) {
    return 0; /* file absent or unreadable */
  }
  if (signal_val <= ctx->last_signal) {
    return 0; /* no new build */
  }
  ...
  if (!new_handle) {
    ...
    DWORD code = GetLastError();
    char err_buf[256];
    snprintf(err_buf, sizeof(err_buf), "LoadLibraryA failed (error %lu): %s", code, new_copy);
    set_error(err_buf);
#else
    set_error(dlerror());
#endif
