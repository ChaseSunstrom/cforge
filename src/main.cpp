...
/**
 * @brief Initialize the CForge context with command line arguments and environment settings
 */
cforge_int_t cforge_init_context(cforge_int_t argc, cforge_string_t argv[], cforge_context_t* ctx) {
    // Initialize context
...
// Win32 entry point
#ifdef _WIN32
cforge_int_t WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, cforge_int_t nCmdShow) {
    // Get command line arguments using Windows API
    cforge_int_t argc;
    LPWSTR* argvW = CommandLineToArgvW(GetCommandLineW(), &argc);
    
    if (argvW == NULL) {
        MessageBoxA(NULL, "Failed to parse command line", "CForge Error", MB_ICONERROR);
        return 1;
    }
    
    // Convert wide char arguments to regular char
    cforge_string_t* argv = (cforge_string_t*)malloc(argc * sizeof(cforge_string_t));
    if (!argv) {
        LocalFree(argvW);
        return 1;
    }
    ...
    for (cforge_int_t i = 0; i < argc; i++) {
        cforge_int_t size_needed = WideCharToMultiByte(CP_UTF8, 0, argvW[i], -1, NULL, 0, NULL, NULL);
