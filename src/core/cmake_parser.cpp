...
static void extract_project(const std::string &content, cmake_parse_result &result) {
  try {
    ...
    std::regex re_project(R"(project\s*\(([^)]+)\))", std::regex::icase);
...
static void extract_cpp_standard(const std::string &content, cmake_parse_result &result) {
  try {
    ...
    std::regex re_set(R"(set\s*\(\s*CMAKE_CXX_STANDARD\s+(\d+)\s*\))", std::regex::icase);
    std::smatch m;
...
static void extract_c_standard(const std::string &content, cmake_parse_result &result) {
  try {
    std::regex re_set(R"(set\s*\(\s*CMAKE_C_STANDARD\s+(\d+)\s*\))", std::regex::icase);
    std::smatch m;
    if (std::regex_search(content, m, re_set)) {
...
static void extract_binary_type(const std::string &content, cmake_parse_result &result) {
  try {
    ...
    std::regex re_exe(R"(add_executable\s*\(\s*(\w+))", std::regex::icase);
    std::smatch m_exe;
    ...
    std::regex re_lib(R"(add_library\s*\(\s*(\w+)\s*(STATIC|SHARED|INTERFACE|MODULE)?)",
                      std::regex::icase);
    std::smatch m_lib;
    if (std::regex_search(content, m_lib, re_lib)) {
      ...
      if (!found_executable) {
        ...
        if (lib_type == "SHARED") {
          ...
          result.binary_type = "shared";
          result.warnings.push_back("MODULE library mapped to binary_type = "
                                    "\"shared\" (closest match)");
        } else {
          // STATIC or empty defaults to static
          result.binary_type = "static";
        }
      } else {
        result.warnings.push_back("Both add_executable and add_library found; "
                                  "using executable target only");
      }
    }
...
static void extract_source_dirs(const std::string &content,
  ...
  try {
    ...
    std::regex re_glob(R"(file\s*\(\s*GLOB(?:_RECURSE)?\s+\w+\s+([^)]+)\))", std::regex::icase);
    ...
    for (; it != end_it; ++it) {
      ...
      for (auto &tok : tokens) {
        // Skip CMake keywords
        if (tok == "CONFIGURE_DEPENDS" || tok == "LIST_DIRECTORIES" || tok == "true"
            || tok == "false") {
          continue;
        }
        ...
        if (dir_part.find("${") != std::string::npos || dir_part.find("$<") != std::string::npos) {
          continue;
        }
    ...
    if (!found_any) {
      std::regex re_src(R"(target_sources\s*\(\s*\w+\s+(?:PRIVATE|PUBLIC|INTERFACE)\s+([^)]+)\))",
                        std::regex::icase);
      ...
      for (; it2 != end_it; ++it2) {
        ...
        for (auto &tok : tokens) {
          if (tok == "PRIVATE" || tok == "PUBLIC" || tok == "INTERFACE") {
            continue;
          }
    ...
    if (!found_any) {
      ...
      if (std::filesystem::exists(src_dir) && std::filesystem::is_directory(src_dir)) {
        ...
        result.warnings.push_back("No explicit source globs found; defaulting to src/");
      }
    }
...
/// Extract include directories from target_include_directories() and
/// include_directories().
static void extract_include_dirs(const std::string &content, cmake_parse_result &result) {
  try {
    ...
    for (; it != end_it; ++it) {
      ...
      auto tokens      = tokenize_cmake_args(args);
      for (auto &tok : tokens) {
        if (tok == "PUBLIC" || tok == "PRIVATE" || tok == "INTERFACE") {
          continue;
        }
    ...
    std::regex re_id(R"(include_directories\s*\(\s*([^)]+)\))", std::regex::icase);
    ...
    for (; it2 != end_it; ++it2) {
      ...
      auto tokens      = tokenize_cmake_args(args);
      for (auto &tok : tokens) {
        if (tok == "BEFORE" || tok == "AFTER" || tok == "SYSTEM") {
          continue;
        }
        if (tok.find("${") != std::string::npos) {
          continue;
...
/// Extract dependencies from find_package(), FetchContent_Declare(),
/// add_subdirectory().
static void extract_dependencies(const std::string &content, cmake_parse_result &result) {
  ...
  try {
    ...
    // Match FetchContent_Declare( ... ) blocks (multiline-friendly since we
    // joined lines)
    std::regex re_fc(R"(FetchContent_Declare\s*\(\s*(\w+)\s+([^)]+)\))", std::regex::icase);
    ...
    for (; it != end_it; ++it) {
      ...
      if (std::regex_search(body, m_tag, re_tag)) {
        ...
        result.warnings.push_back("FetchContent dependency '" + dep.name
                                  + "' has no GIT_TAG — using \"latest\"");
      }
  ...
  try {
    // --- add_subdirectory ---
    std::regex re_sub(R"(add_subdirectory\s*\(\s*([^)\s]+))", std::regex::icase);
...
/// Extract compile definitions from target_compile_definitions() and
/// add_definitions().
static void extract_compile_definitions(const std::string &content, cmake_parse_result &result) {
  try {
    ...
    std::regex re_ad(R"(add_definitions\s*\(\s*([^)]+)\))", std::regex::icase);
...
static void check_skipped_constructs(const std::string &content, cmake_parse_result &result) {
  try {
    ...
    auto end_it = std::sregex_iterator();
    for (; it != end_it; ++it) {
      result.warnings.push_back("Custom CMake function '" + (*it)[1].str() + "' was skipped");
    }
    ...
    for (; it2 != end_it; ++it2) {
      result.warnings.push_back("Custom CMake macro '" + (*it2)[1].str() + "' was skipped");
    }
    ...
    for (; it3 != end_it; ++it3) {
      ...
      if (mod == "FetchContent" || mod == "ExternalProject" || mod == "GNUInstallDirs"
          || mod == "CMakePackageConfigHelpers" || mod == "CPack") {
        continue;
      }
