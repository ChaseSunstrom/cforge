...
#endif
{
  ...
  if (!GetConsoleMode(m_stdin_handle, &m_saved_mode)) {
    return;
  }
  DWORD raw_mode = m_saved_mode & ~(ENABLE_LINE_INPUT | ENABLE_ECHO_INPUT);
...
static void print_option_line(const std::string &option, bool selected, bool clear_first) {
  if (clear_first) {
  ...
  if (selected) {
    ...
    fmt::print(fg(fmt::color::green) | fmt::emphasis::bold, "  {:>{}} ", ">", STATUS_WIDTH - 2);
...
cforge_int_t prompt_select(const std::string &label,
  ...
  fmt::print(fg(fmt::color::green) | fmt::emphasis::bold, "{:>{}}", label, STATUS_WIDTH);
  fmt::print(":\n");
  ...
  for (;;) {
    ...
    if (ev.key == Key::Up) {
      ...
      selected = (selected + 1 < static_cast<int>(options.size())) ? selected + 1 : 0;
    } else {
      // Ignore other keys.
      continue;
    }
