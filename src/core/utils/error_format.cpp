...
std::string format_build_errors(const std::string &error_output) {
  ...
  for (const auto &diag : filtered_diagnostics) {
    ...
    if (diag.occurrence_count > 1) {
      ...
      diag_copy.message    = diag.message + " (" + std::to_string(diag.occurrence_count)
                        + " occurrences)";
...
// This function maintains the same formatting as logger utilities but returns a
// string for use in contexts where we need to collect/process output before
// printing.
std::string format_diagnostic_to_string(const diagnostic &diag) {
  ...
  // Code snippet with line numbers - matches logger::print_code_line() and
  // print_error_pointer()
...
void print_diagnostic(const diagnostic &diag) {
  ...
  switch (diag.level) {
    ...
    case diagnostic_level::WARNING:
      ...
      break;
    case diagnostic_level::NOTE:
      logger::print_diag_note(diag.code.empty() ? diag.message
                                                : "[" + diag.code + "]: " + diag.message);
      break;
    case diagnostic_level::HELP:
      logger::print_diag_help(diag.code.empty() ? diag.message
                                                : "[" + diag.code + "]: " + diag.message);
      break;
    default:
  ...
  if (!diag.fixes.empty()) {
    ...
    if (diag.fixes.size() > 3) {
      logger::print_dim("            ... and " + std::to_string(diag.fixes.size() - 3)
                        + " more suggestion(s)");
    }
  }
...
std::vector<diagnostic> parse_gcc_clang_errors(const std::string &error_output) {
  ...
  //   file.cpp:10:15: warning: format '%d' expects ... type 'std::string'
  //   [-Wformat=]
  //
    ...
    if (std::regex_search(line, matches, error_regex)) {
      ...
      switch (std::stoi(diag.code.substr(1))) {  // Remove the C prefix and convert to int
        ...
        case 3861:  // C3861: 'identifier': identifier not found
          diag.help_text = "Function not found. Check for typos, missing includes, or if the "
...
std::vector<diagnostic> parse_cmake_errors(const std::string &error_output) {
  ...
  std::regex error_at_regex(R"(CMake\s+(Error|Warning)\s+at\s+([^:]+):(\d+)\s+\(([^)]+)\):\s*$)");
  ...
  while (std::getline(stream, line)) {
    ...
    if (std::regex_search(line, match_at, error_at_regex)) {
      ...
      diag.level     = (level_str == "Error") ? diagnostic_level::ERROR : diagnostic_level::WARNING;
...
std::vector<diagnostic> parse_linker_errors(const std::string &error_output) {
  ...
  std::regex msvc_obj_error_regex(R"(([^\s:]+\.obj)\s*:\s*(?:fatal\s*)?error\s*(LNK\d+):\s*(.*))");

  // MSVC linker error with function context: "file.obj : error LNK2019: ... in
  // function ..."
  std::regex msvc_function_context_regex(R"(function\s+[\"']?([^\"'\s]+)[\"']?)");
  ...
  // `symbol'"
  std::regex simple_undefined_ref_regex(R"(undefined reference to [`']([^'`]+)[`'])");
      ...
      try {
        ...
        switch (code_num) {
          ...
          case 1181:
            ...
            break;
          case 2001:
            diag.help_text = "Unresolved external symbol. The symbol is declared but not "
                             "defined. Check:\n"
          ...
          case 2005:
            ...
            break;
          case 2019:
            diag.help_text = "Unresolved external symbol referenced in function. The function "
  ...
  while (std::getline(stream, line)) {
    ...
    // Then try to match CPack errors embedded in CMake errors
      ...
      if (message.find("Error when generating package") != std::string::npos) {
        diag.help_text = "Check that you have specified valid generators in your package "
...
std::vector<diagnostic> parse_template_errors(const std::string &error_output) {
  ...
  std::regex candidate_regex(R"(([^:]+):(\d+):(\d+):\s*note:\s*candidate:\s*(.*))");
    ...
    if (current_template_error != nullptr && instantiation_depth < MAX_INSTANTIATION_DEPTH) {
      if (std::regex_search(line, matches, gcc_instantiation_regex)
        ...
        if (!context.empty()) {
          ...
          current_template_error->notes.push_back("instantiated from " + file + ":" + line_num);
          instantiation_depth++;
        }
        continue;
      }
...
std::vector<diagnostic> deduplicate_diagnostics(std::vector<diagnostic> diagnostics) {
  ...
  auto get_dedup_key = [](const diagnostic &d) -> std::string {
    ...
    if (d.code.find("LNK") != std::string::npos || d.code.find("UNDEFINED") != std::string::npos) {
    ...
    // This handles both "Undeclared identifier: foo" and "'foo' was not
    // declared"
    if (d.code.find("UNDECL") != std::string::npos || d.code.find("UNDECLARED") != std::string::npos
    ...
    if (!d.file_path.empty() && d.line_number > 0) {
      // Use file:line:column as the primary key - this deduplicates across
      // parsers that report the same diagnostic with different codes (GCC, ICX,
      // clang-tidy, etc.)
      ...
      // Include diagnostic level to distinguish errors from warnings at same
      // location
    ...
    if (it != seen_errors.end()) {
      ...
      if (new_has_better_info) {
        ...
        const bool different_line = looks_like_path(diag.file_path)
                                 && diag.file_path == existing.file_path && diag.line_number > 0
...
error_summary calculate_error_summary(const std::vector<diagnostic> &diagnostics) {
  ...
  for (const auto &diag : diagnostics) {
    ...
    if (diag.code.find("LNK") != std::string::npos || diag.code.find("LD") != std::string::npos
        || diag.code.find("UNDEFINED") != std::string::npos
  ...
  std::sort(summary.error_categories.begin(),
            summary.error_categories.end(),
...
std::string format_error_summary(const error_summary &summary) {
  ...
  if (summary.compiler_errors > 0) {
    ss << fmt::format(fg(fmt::color::steel_blue), "   |  ");
    ss << fmt::format(fg(fmt::color::red),
                      "{} compiler error{}\n",
...
std::vector<fix_suggestion> generate_fix_suggestions(const diagnostic &diag) {
  ...
  if (msg_lower.find("expected ';'") != std::string::npos
      ...
      || msg_lower.find("missing ';'") != std::string::npos || diag.code == "C2143"
      || diag.code == "C2146") {
    fix_suggestion fix;
    ...
    if (!diag.line_content.empty()) {
      ...
      if (end != std::string::npos) {
        fix.start_column = static_cast<cforge_int_t>(end + 2);  // After last non-space char
      }
    }
  ...
  if (msg_lower.find("expected '}'") != std::string::npos
      || msg_lower.find("missing '}'") != std::string::npos || diag.code == "C2059") {
    fix_suggestion fix;
  ...
  if (msg_lower.find("undeclared") != std::string::npos
      ...
      || msg_lower.find("does not name a type") != std::string::npos || diag.code == "C2065"
      || diag.code == "C3861") {
  ...
  if (msg_lower.find("no return statement") != std::string::npos
      ...
      || msg_lower.find("not all control paths return") != std::string::npos || diag.code == "C4715"
      || diag.code == "C4716") {
    fix_suggestion fix;
  ...
  if (msg_lower.find("implicit conversion") != std::string::npos
      ...
      || msg_lower.find("possible loss of data") != std::string::npos || diag.code == "C4244"
      || diag.code == "C4267") {
    fix_suggestion fix;
...
std::vector<diagnostic> parse_sanitizer_errors(const std::string &error_output) {
  ...
  std::regex asan_error_regex(R"(==\d+==ERROR:\s*AddressSanitizer:\s*([^\n]+))");

  // ASan stack trace location
  // #0 0x... in function_name file.cpp:123
  std::regex asan_frame_regex(R"(#(\d+)\s+0x[0-9a-fA-F]+\s+in\s+(\S+)\s+([^:]+):(\d+))");

  // UndefinedBehaviorSanitizer
  // file.cpp:123:45: runtime error: signed integer overflow
  std::regex ubsan_regex(R"(([^:]+):(\d+):(\d+):\s*runtime error:\s*(.+))");

  // ThreadSanitizer
  // WARNING: ThreadSanitizer: data race (pid=12345)
  std::regex tsan_regex(R"(WARNING:\s*ThreadSanitizer:\s*([^\(]+))");

  // MemorySanitizer
  // ==12345==WARNING: MemorySanitizer: use-of-uninitialized-value
  std::regex msan_regex(R"(==\d+==WARNING:\s*MemorySanitizer:\s*([^\n]+))");

  // LeakSanitizer
  // ==12345==ERROR: LeakSanitizer: detected memory leaks
  std::regex lsan_regex(R"(==\d+==ERROR:\s*LeakSanitizer:\s*([^\n]+))");
...
std::vector<diagnostic> parse_assertion_errors(const std::string &error_output) {
  ...
  std::regex gcc_assert_regex(R"(([^:]+):(\d+):\s*(\S+):\s*Assertion\s*[`']([^'`]+)[`']\s*failed)");

  // MSVC runtime assert
  // Debug Assertion Failed! ... File: file.cpp, Line: 123
  std::regex msvc_assert_regex(R"(Debug Assertion Failed.*File:\s*([^,]+),\s*Line:\s*(\d+))");
...
std::vector<diagnostic> parse_module_errors(const std::string &error_output) {
  ...
  while (std::getline(stream, line)) {
    ...
    if (std::regex_search(line, matches, module_interface_regex)) {
      ...
      diag.help_text     = "Module interface compilation failed. Check for errors "
...
std::vector<diagnostic> parse_runtime_errors(const std::string &error_output) {
  ...
  std::regex uncaught_regex(R"(terminate called after throwing an instance of '([^']+)')");
...
std::vector<diagnostic> parse_test_framework_errors(const std::string &error_output) {
  ...
  //   y
  //     Which is: 2
  std::regex gtest_failure_regex(R"(([^:]+):(\d+):\s*Failure)");
  ...
  std::regex gtest_expect_regex(R"(^([^:\n]+\.[A-Za-z][\w]*):(\d+):\s+error:\s+(.+))");

  // Google Test death test
  std::regex gtest_death_regex(R"(Death test:\s*(.+))");
  ...
  // with expansion:
  //   1 == 2
  std::regex catch2_failure_regex(R"(([^:]+):(\d+):\s*FAILED:)");
  ...
  std::regex gtest_failed_test_regex(R"(\[\s*FAILED\s*\]\s*(\S+)\s*\((\d+)\s*ms\))");
...
std::vector<diagnostic> parse_static_analysis_errors(const std::string &error_output) {
  ...
  std::regex pvs_regex(R"(([^:]+):(\d+):(\d+):\s*(error|warning|note):\s*(V\d+)\s*(.+))");
...
std::vector<diagnostic> parse_concept_errors(const std::string &error_output) {
  ...
  // MSVC concept error
  // file.cpp(123): error C7602: 'concept_name': the associated constraints are
  // not satisfied
  std::regex msvc_concept_regex(R"(([^\(]+)\((\d+)\):\s*error\s+C7602:\s*'([^']+)':\s*(.+))");
...
std::vector<diagnostic> parse_constexpr_errors(const std::string &error_output) {
  ...
  std::regex constexpr_expansion_regex(R"(in\s*'constexpr'\s*expansion\s*of\s*'([^']+)')");
  ...
  std::regex msvc_constexpr_regex(R"(([^\(]+)\((\d+)\):\s*error\s+C2131:\s*(.+))");
...
std::vector<diagnostic> parse_coroutine_errors(const std::string &error_output) {
  ...
  std::regex msvc_await_regex(R"(([^\(]+)\((\d+)\):\s*error\s+C3312:\s*(.+))");

  // error C3313: 'identifier': variable cannot have type 'coroutine type'
  std::regex msvc_coro_type_regex(R"(([^\(]+)\((\d+)\):\s*error\s+C3313:\s*(.+))");
  ...
  while (std::getline(stream, line)) {
    ...
    if (std::regex_search(line, matches, range_concept_regex)) {
      ...
      if (concept_name == "range" || concept_name == "input_range") {
        ...
        diag.help_text = "Ensure the type meets all requirements for the " + concept_name
                       + " concept.";
      }
      diagnostics.push_back(diag);
      continue;
    ...
    if (std::regex_search(line, matches, begin_end_regex)) {
      ...
      diag.help_text     = "Add a " + matches[3].str()
                     + "() member function or a free function "
                       "findable via ADL. Consider using "
                       "std::ranges::begin/end customization points.";
      diagnostics.push_back(diag);
      continue;
    }
    ...
    if (std::regex_search(line, matches, iterator_regex)) {
      ...
      diag.message          = "Type '" + matches[3].str() + "' does not satisfy '" + iter_type
                   + "_iterator'";
      diag.help_text = "Iterator requirements for " + iter_type
                     + "_iterator:\n"
...
std::vector<diagnostic> parse_cuda_hip_errors(const std::string &error_output) {
  ...
  std::regex nvcc_error_regex(R"(([^\(]+)\((\d+)\):\s*error:\s*(.+))");

  // NVCC warnings
  std::regex nvcc_warning_regex(R"(([^\(]+)\((\d+)\):\s*warning:\s*(.+))");

  // CUDA kernel launch errors
  std::regex kernel_launch_regex(R"(([^\(]+)\((\d+)\):\s*error:.*(?:kernel|__global__|<<<|>>>).*)");
  ...
  std::regex hip_error_regex(R"(([^:]+):(\d+):(?:\d+:)?\s*error:.*(?:hip|HIP|__hip_|hipLaunch))");

  // CUDA architecture mismatch
  std::regex arch_regex(R"((?:error|warning):.*(?:sm_\d+|compute_\d+|arch=|gencode).*)");
  ...
  // PTX assembly errors
  std::regex ptx_regex(R"(ptxas\s*(?:error|fatal):?\s*(.+))");
...
std::vector<diagnostic> parse_intel_compiler_errors(const std::string &error_output) {
  ...
  std::regex icc_error_regex(R"(([^\(]+)\((\d+)\):\s*error(?:\s*#(\d+))?:\s*(.+))");

  // Classic ICC warning format
  std::regex icc_warning_regex(R"(([^\(]+)\((\d+)\):\s*warning(?:\s*#(\d+))?:\s*(.+))");

  // ICX (Intel oneAPI) uses Clang-like format: file.cpp:123:45: error: message
  std::regex icx_error_regex(R"(([^:]+):(\d+):(\d+):\s*error:\s*(.+))");

  // ICX warning
  std::regex icx_warning_regex(R"(([^:]+):(\d+):(\d+):\s*warning:\s*(.+))");

  // Intel-specific remarks (optimization reports)
  std::regex remark_regex(R"(([^\(]+)\((\d+)\):\s*remark(?:\s*#(\d+))?:\s*(.+))");
  ...
  std::regex openmp_regex(R"(([^\(]+)\((\d+)\):\s*error.*(?:omp|OMP|openmp|OpenMP)\s*(.+))");

  // Intel MKL/IPP linking errors
  std::regex mkl_regex(R"((?:error|undefined).*(?:mkl_|MKL_|ipp|IPP)\w+)");

  // SIMD intrinsics errors
  std::regex simd_regex(R"(([^\(]+)\((\d+)\):\s*error:.*(?:_mm|__m\d+|_mm\d+|avx|AVX|sse|SSE))");
...
std::vector<diagnostic> parse_pch_errors(const std::string &error_output) {
  ...
  std::regex msvc_pch_error_regex(R"(([^\(]+)(?:\((\d+)\))?:\s*error\s+(C1[89]\d\d):\s*(.+))");
  ...
  std::regex clang_include_pch_regex(R"(([^:]+):(?:(\d+):)?(?:\d+:)?\s*error:.*-include-pch.*)");
...
std::vector<diagnostic> parse_abi_errors(const std::string &error_output) {
  ...
  std::regex triple_regex(R"((?:error|warning):.*(?:target\s*triple|unknown\s*target|-target))");
