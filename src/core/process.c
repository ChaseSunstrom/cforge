 ...
 */
 ...
 #else
     if (process->stdout_handle) {
         close((int)(intptr_t)process->stdout_handle);
         process->stdout_handle = NULL;
     }
     if (process->stderr_handle) {
         close((int)(intptr_t)process->stderr_handle);
         process->stderr_handle = NULL;
     }
 #endif
 }
 ...
 cforge_process_status_t cforge_process_start(cforge_process_t* process,
     ...
     si.hStdOutput = stdout_redirect != CFORGE_REDIRECT_NONE ? stdout_write : GetStdHandle(STD_OUTPUT_HANDLE);
     si.hStdError = stderr_redirect != CFORGE_REDIRECT_NONE ? stderr_write : GetStdHandle(STD_ERROR_HANDLE);
 ...
 cforge_process_status_t cforge_process_start(cforge_process_t* process,
     ...
     if (pid == 0) {
         ...
         cforge_char_t** args = (cforge_char_t**)malloc((process->arg_count + 2) * sizeof(cforge_char_t*));
         if (!args) {
             _exit(EXIT_FAILURE);
         }
 ...
 cforge_process_status_t cforge_run_command(cforge_cstring_t command,
     ...
     status = cforge_process_start(&process, working_dir, 
                                 CFORGE_REDIRECT_PIPE, CFORGE_REDIRECT_PIPE);
     if (status != CFORGE_PROC_SUCCESS) {
     ...
     if (!output->stdout_data || !output->stderr_data) {
         if (output->stdout_data) free(output->stdout_data);
         if (output->stderr_data) free(output->stderr_data);
         output->stdout_data = NULL;
         output->stderr_data = NULL;
