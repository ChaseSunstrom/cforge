...
static std::vector<std::filesystem::path> find_all_build_dirs(const std::string& base_dir) {
    ...
    for (const auto& config : configs) {
        std::filesystem::path config_dir = get_build_dir_for_config(base_dir, config);
...
static bool clean_build_directory(const std::filesystem::path& build_dir, bool verbose) {
    if (!std::filesystem::exists(build_dir)) {
        logger::print_status("Build directory does not exist, nothing to clean: " + build_dir.string());
        return true;
    }
    ...
    try {
        ...
        logger::print_success("Cleaned " + std::to_string(count) + " files/directories from " + build_dir.string());
        return true;
    } catch (const std::exception& e) {
        logger::print_error("Failed to clean build directory: " + std::string(e.what()));
        return false;
    }
}
 ...
 * @return bool Success flag
 */
static bool regenerate_cmake_files(
...
cforge_int_t cforge_cmd_clean(const cforge_context_t* ctx) {
    ...
    if (regenerate) {
        std::filesystem::path build_dir = get_build_dir_for_config(base_build_dir, config_name);
