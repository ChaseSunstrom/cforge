 ...
 */
static std::filesystem::path find_test_executable(
    ...
    const std::string& test_executable_name = "") 
{
    ...
    for (const auto& path : search_paths) {
        if (std::filesystem::exists(path)) {
            logger::print_status("Found test executable with expected name: " + path.string());
            return path;
        }
    }
    ...
    for (const auto& alt_base : alt_names) {
        ...
        for (const auto& base_path : {
            ...
            if (std::filesystem::exists(alt_path)) {
                logger::print_status("Found alternative test executable: " + alt_path.string());
                return alt_path;
            }
        }
    }
    
    // If still not found, do a recursive search for any executable that might be a test
    ...
    for (const auto& entry : std::filesystem::recursive_directory_iterator(build_dir)) {
        if (entry.is_regular_file()) {
            ...
            if (lower_filename.find("test") != std::string::npos) {
                ...
                #endif
                    logger::print_status("Found test executable via search: " + entry.path().string());
                    return entry.path();
                }
            }
        }
    ...
    int jobs)
{
    ...
    auto result = execute_tool(command, args, build_dir.string(), "CTest", verbose);
    ...
    bool verbose)
{
    ...
    logger::print_status("Running tests with " + test_executable.filename().string());
    auto result = execute_tool(command, test_args, working_dir.string(), "Test", verbose);
...
cforge_int_t cforge_cmd_test(const cforge_context_t* ctx)
{
    ...
    if (!config.has_key("project.name") || project_name.empty()) {
        ...
        if (project_name.empty()) {
            logger::print_error("Project name not found in " + std::string(CFORGE_FILE));
            return 1;
        }
    }
    ...
    std::string test_executable_name = config.get_string("test.test_executable", "");
    
    auto test_executable = find_test_executable(build_dir, project_name, build_config, test_executable_name);
    
    if (test_executable.empty()) {
        logger::print_error("Test executable not found");
        logger::print_status("Expected test executable format: " + project_name + "_" + build_config + "_tests");
        return 1;
    }
