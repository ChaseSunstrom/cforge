...
static std::filesystem::path get_vcpkg_path(const toml_reader* project_config) {
    ...
    if (project_config && project_config->has_key("dependencies.vcpkg_path")) {
        ...
        if (vcpkg_path.string().find('%') != std::string::npos) {
            ...
            ExpandEnvironmentStringsA(vcpkg_path.string().c_str(), expanded_path, MAX_PATH);
 ...
 */
static bool clone_vcpkg(const std::filesystem::path& project_dir, bool verbose) {
    std::filesystem::path vcpkg_dir = project_dir / "vcpkg";
    ...
    try {
        ...
        logger::print_error("Failed to create vcpkg directory: " + std::string(e.what()));
        return false;
    }
 ...
 * @return true if successful, false otherwise
 */
static bool setup_vcpkg_integration(const std::filesystem::path& project_dir, bool verbose) {
    std::filesystem::path vcpkg_dir = project_dir / "vcpkg";
    std::filesystem::path vcpkg_exe;
    ...
    if (ctx->args.args && ctx->args.args[0]) {
        if (strcmp(ctx->args.args[0], "setup") == 0) {
            ...
            if (is_vcpkg_installed(project_dir)) {
                logger::print_status("vcpkg is already installed in the project directory");
