 ...
 */
static bool clone_git_repo(const std::string& url, const std::string& target_dir, const std::string& tag, bool verbose) {
    ...
    if (std::filesystem::exists(target_dir)) {
        ...
        if (!fetch_result.success) {
            logger::print_warning("Failed to fetch updates, continuing with existing version");
        }
    ...
    try {
        ...
        logger::print_error("Failed to create target directory: " + std::string(e.what()));
        return false;
    }
    ...
    if (!result.success) {
        logger::print_error("Git clone failed with exit code: " + std::to_string(result.exit_code));
        return false;
    }
...
static bool add_dependency_to_section(const std::filesystem::path &config_file,
    ...
    if (verbose) {
        logger::print_verbose("Added dependency to section [" + section + "]: " + entry);
    }
...
cforge_int_t cforge_cmd_add(const cforge_context_t *ctx) {
  ...
  for (size_t i = 0; i < args.size(); ++i) {
    if (args[i] == "--git") mode_git = true;
    else if (args[i] == "--vcpkg") mode_vcpkg = true;
    else if (args[i] == "--tag") {
      if (i + 1 < args.size()) {
        tag_value = args[i + 1];
    ...
    else filtered.push_back(args[i]);
  }
  args.swap(filtered);
  ...
  if (in_workspace_root) {
    ...
    for (const auto &proj : projects) {
      ...
      if (mode_git) {
        ...
        cfg_ok = add_git_dependency_to_config(proj_dir, proj_config, package_name, package_version_or_url, tag_value, verbose);
        inst_ok = clone_git_repo(package_version_or_url, deps_path.string(), tag_value, verbose);
      } else {
    ...
    if (!cfg_all) {
      logger::print_error("Failed to update configuration for dependency: " + package_name + " in some workspace projects");
      return 1;
    }
    if (!inst_all) {
  ...
  // Single project addition
  {
    ...
    if (mode_git) {
      ...
      cfg_ok = add_git_dependency_to_config(project_dir, config_file, package_name, package_version_or_url, tag_value, verbose);
      inst_ok = clone_git_repo(package_version_or_url, deps_path.string(), tag_value, verbose);
    } else {
    ...
    if (!cfg_ok) {
      logger::print_error("Failed to update configuration for dependency: " + package_name);
      return 1;
    }
    if (!inst_ok) {
