...
process_result execute_process(
    ...
    while (true) {
        ...
        if (timeout_seconds > 0 && elapsed_seconds > timeout_seconds) {
            logger::print_warning("Process timed out after " + std::to_string(timeout_seconds) + " seconds");
            // Terminate the process
            TerminateProcess(pi.hProcess, 1);
            timed_out = true;
            result.exit_code = -1;
            result.success = false;
            result.stderr_output = "Process timed out after " + std::to_string(timeout_seconds) + " seconds";
            break;
        }
        ...
        if (stdout_success && stdout_avail > 0) {
            if (ReadFile(stdout_read, buffer.data(), BUFFER_SIZE - 1, &bytes_read, NULL) && bytes_read > 0) {
                buffer[bytes_read] = '\0';
        ...
        if (stderr_success && stderr_avail > 0) {
            if (ReadFile(stderr_read, buffer.data(), BUFFER_SIZE - 1, &bytes_read, NULL) && bytes_read > 0) {
                buffer[bytes_read] = '\0';
        ...
        // If process has exited and no more data to read, or if the pipes are broken, break
        if (result.exit_code != -1) {
            ...
            // If it's been more than 2 seconds since we've seen any data and process is done,
            // assume we're finished even if PeekNamedPipe might be stuck
...
bool is_command_available(const std::string& command, int timeout_seconds) {
    ...
    // Some tools don't respond well to --version, so we use command-specific flags
    if (command == "git") {
        args = {"--version"};
        ...
        logger::print_verbose("Checking availability of CMake with timeout: " + std::to_string(timeout_seconds) + "s");
#endif
    ...
    try {
        ...
        logger::print_verbose("Checking availability of command: " + command + " (timeout: " + std::to_string(timeout_seconds) + "s)");
        process_result result = execute_process(command, args, "", nullptr, nullptr, timeout_seconds);
        
        if (result.success) {
            logger::print_verbose("Command '" + command + "' is available (" + result.stdout_output.substr(0, 50) + (result.stdout_output.length() > 50 ? "..." : "") + ")");
            return true;
        } else {
            if (result.exit_code == -1) {
                logger::print_verbose("Command '" + command + "' failed: Process execution error");
            } else {
                logger::print_verbose("Command '" + command + "' failed with exit code: " + std::to_string(result.exit_code));
            }
            
            if (!result.stderr_output.empty()) {
                logger::print_verbose("Error output: " + result.stderr_output.substr(0, 100) + (result.stderr_output.length() > 100 ? "..." : ""));
            }
            
            return false;
        }
    } catch (const std::exception& ex) {
        logger::print_verbose("Exception checking command '" + command + "': " + ex.what());
        return false;
    } catch (...) {
