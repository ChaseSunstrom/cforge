...
cforge_int_t cforge_cmd_help(const cforge_context_t* ctx) {
    ...
    if (specific_command.empty()) {
        ...
        logger::print_status("");
        logger::print_status("Special dependency handling:");
        logger::print_status("- Git dependencies are cloned to the specified 'dependencies.directory' or 'deps' by default");
        logger::print_status("- Header-only libraries (like nlohmann/json) are automatically detected");
        logger::print_status("- Some libraries receive special handling:");
        logger::print_status("  * fmt: Linked as fmt::fmt with appropriate CMake options");
        logger::print_status("  * spdlog: Standard CMake target");
        logger::print_status("  * json/nlohmann_json: Header-only, no linking required");
        logger::print_status("");
