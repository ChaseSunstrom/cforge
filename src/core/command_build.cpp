...
static bool generate_cmakelists_from_toml(
    ...
    std::filesystem::path cmakelists_path = build_dir / "CMakeLists.txt";
    ...
    if (file_exists) {
        ...
        logger::print_verbose("CMakeLists.txt already exists in build dir, using existing file");
        return true;
    }
    ...
    // Create CMakeLists.txt in the build directory
    std::ofstream cmakelists(cmakelists_path);
    if (!cmakelists.is_open()) {
        logger::print_error("Failed to create CMakeLists.txt in build directory");
        return false;
    }
    ...
    cmakelists << "set(SOURCE_DIR \"" << project_dir.string() << "\")\n\n";
    ...
    if (project_config.has_key("project.additional_sources")) {
        ...
        if (!additional_sources.empty()) {
            ...
            for (const auto& source : additional_sources) {
                cmakelists << "file(GLOB_RECURSE ADDITIONAL_SOURCES_" << source << " \"${SOURCE_DIR}/" << source << "\")\n";
    ...
    if (project_config.has_key("project.additional_includes")) {
        ...
        if (!additional_includes.empty()) {
            ...
            for (const auto& include : additional_includes) {
                if (binary_type == "header_only") {
                    cmakelists << "target_include_directories(${PROJECT_NAME} INTERFACE \"${SOURCE_DIR}/" << include << "\")\n";
                } else {
                    cmakelists << "target_include_directories(${PROJECT_NAME} PUBLIC \"${SOURCE_DIR}/" << include << "\")\n";
                }
            }
            cmakelists << "\n";
        }
    ...
    if (project_config.has_key("project.definitions")) {
        ...
        if (!definitions.empty()) {
            ...
            for (const auto& def : definitions) {
                if (binary_type == "header_only") {
                    cmakelists << "target_compile_definitions(${PROJECT_NAME} INTERFACE " << def << ")\n";
                } else {
    ...
    if (project_config.has_key("dependencies.git")) {
        ...
        for (const auto& dep : git_deps) {
            ...
            if (link) {
                // Exceptions for special libraries
                if (dep == "fmt") {
                    cmakelists << "    fmt::fmt\n";
                } else if (dep != "json" && dep != "nlohmann_json") {
                    // json and nlohmann_json are header-only
                    cmakelists << "    " << dep << "\n";
                }
            }
        }
    ...
    if (binary_type == "header_only") {
        ...
        cmakelists << "else()\n";
        cmakelists << "    target_compile_options(${PROJECT_NAME} PRIVATE -Wall -Wextra -Wpedantic)\n";
        cmakelists << "endif()\n\n";
    }
    ...
    if (std::filesystem::exists(tests_dir) && std::filesystem::is_directory(tests_dir)) {
        ...
        cmakelists << "    add_subdirectory(\"${SOURCE_DIR}/tests\" ${CMAKE_BINARY_DIR}/tests)\n";
        cmakelists << "endif()\n\n";
    } else {
    ...
    if (binary_type == "executable") {
        ...
        cmakelists << "    install(FILES \"${CMAKE_BINARY_DIR}/bin/${CMAKE_BUILD_TYPE}/$<TARGET_FILE_NAME:${PROJECT_NAME}>.pdb\"\n";
    ...
    cmakelists << "set(CPACK_RESOURCE_FILE_README \"${SOURCE_DIR}/README.md\")\n\n";
    ...
    cmakelists << "set(CPACK_PACKAGE_FILE_NAME \"${CPACK_PACKAGE_NAME}-${CPACK_PACKAGE_VERSION}-${CMAKE_SYSTEM_NAME}-${CMAKE_BUILD_TYPE}\")\n\n";
    ...
    cmakelists.close();
    logger::print_verbose("Generated CMakeLists.txt in build directory: " + cmakelists_path.string());
    return true;
}
...
static bool run_cmake_configure(
    ...
    if (!has_build_type) {
        logger::print_warning("No build type specified in CMake arguments - this should not happen");
    }
...
static bool build_project(
    ...
    if (!use_workspace_build && has_project_config) {
        ...
        if (!generate_cmakelists_from_toml(project_dir, project_config, verbose)) {
            logger::print_error("Failed to generate CMakeLists.txt in build directory");
            return false;
        }
        ...
        logger::print_success("Generated CMakeLists.txt file in build directory");
    }
    
    // Prepare CMake arguments
    std::vector<std::string> cmake_args;
    
    // Use current directory as source for build directory CMakeLists.txt
    cmake_args.push_back(".");
    
    // Always specify the build type, regardless of generator type
    // This ensures it's properly set in CMakeCache.txt
    ...
    // Add config - always specify it for both single-config and multi-config generators
...
cforge_int_t cforge_cmd_build(const cforge_context_t* ctx) {
    ...
    for (int i = 0; i < ctx->args.arg_count; i++) {
        ...
        if (arg == "-c" || arg == "--config") {
            if (i + 1 < ctx->args.arg_count) {
                ...
                logger::print_verbose("Using build configuration from command line: " + config_name);
    ...
    if (config_name.empty() && ctx->args.config != nullptr && strlen(ctx->args.config) > 0) {
        ...
        logger::print_verbose("Using build configuration from context: " + config_name);
    }
