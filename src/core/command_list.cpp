...
static void list_generators() {
    ...
    std::cout << "  - Xcode                      Xcode project files (macOS only)\n";
...
static void list_commands() {
    ...
    std::cout << "\n";
    std::cout << "Run 'cforge help <command>' for more information about a specific command.\n";
    std::cout << "\n";
}
...
static void list_project_settings() {
    ...
    std::cout << "cpp_standard = \"17\"         # Optional: C++ standard version\n";
    std::cout << "\n";
    std::cout << "[build]\n";
    std::cout << "build_dir = \"build\"         # Optional: Build directory name\n";
    ...
    std::cout << "vcpkg_path = \"/path/to/vcpkg\" # Optional: Custom vcpkg path\n";
    std::cout << "\n";
}
...
cforge_int_t cforge_cmd_list(const cforge_context_t* ctx) {
    ...
    if (!category.empty()) {
        if (category == "configs" || category == "configurations") {
            ...
            std::cout << "Available categories: configs, generators, targets, commands, settings\n";
            return 1;
        }
    }
    else {
