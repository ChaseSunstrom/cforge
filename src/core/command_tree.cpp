...
void print_tree_branch(const std::string& name, const dependency_info& info,
                       ...
                       std::set<std::string>& visited, int max_depth, int current_depth) {
  ...
  for (size_t i = 0; i < info.children.size(); i++) {
    ...
    if (it != all_deps.end()) {
      print_tree_branch(child, it->second, all_deps, child_prefix, child_is_last,
                        visited, max_depth, current_depth + 1);
    } else {
      fmt::print("{}{}{}\n", child_prefix, child_is_last ? "`-- " : "|-- ", child);
    }
  }
}
 ...
 */
void collect_dependencies(const fs::path& project_dir, const cforge::toml_reader& config,
...
cforge_int_t cforge_cmd_tree(const cforge_context_t *ctx) {
  ...
  for (const auto& [name, info] : all_deps) {
    if (info.type == "git") git_count++;
    else if (info.type == "vcpkg") vcpkg_count++;
    else if (info.type == "system") sys_count++;
    else if (info.type == "project") proj_count++;
  }

  std::vector<std::string> summary_parts;
  if (git_count > 0) summary_parts.push_back(std::to_string(git_count) + " git");
  if (vcpkg_count > 0) summary_parts.push_back(std::to_string(vcpkg_count) + " vcpkg");
  if (sys_count > 0) summary_parts.push_back(std::to_string(sys_count) + " system");
  if (proj_count > 0) summary_parts.push_back(std::to_string(proj_count) + " project");
