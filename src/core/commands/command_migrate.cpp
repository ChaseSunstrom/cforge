 ...
 * @brief Implementation of the 'migrate' command to import CMakeLists.txt into
 * cforge.toml
 */
...
static std::string generate_cforge_toml(const cforge::cmake_parse_result &r) {
  ...
  if (has_subdir_deps) {
    out << "# add_subdirectory dependencies detected (no automatic cforge "
           "mapping):\n";
    for (const auto &dep : r.dependencies) {
      if (dep.is_subdirectory) {
  ...
  if (!r.link_libraries.empty()) {
    out << "# Additional link libraries detected (review and add manually if "
           "needed):\n";
  ...
  if (!r.compile_options.empty()) {
    out << "# Compiler options detected (add to build.config sections manually "
           "if needed):\n";
  ...
  // -------------------------------------------------------------------------
  out << "# ---- Migration Notes ----\n";
  out << "# Variables (${VAR}) were not expanded; paths may contain variable "
         "references.\n";
...
cforge_int_t cforge_cmd_migrate(const cforge_context_t *ctx) {
  ...
  // Start migration
  // -------------------------------------------------------------------------
  cforge::logger::print_action("Migrating", cmake_path.string() + " -> " + output_fs.string());
  cforge::logger::print_blank();

  // -------------------------------------------------------------------------
  // Parse CMakeLists.txt
  // -------------------------------------------------------------------------
  cforge::cmake_parse_result parse_result = cforge::parse_cmake_file(cmake_path.string());
  ...
  if (!parse_result.source_dirs.empty()) {
    cforge::logger::print_status(
        "Extracted " + std::to_string(parse_result.source_dirs.size()) + " source "
        + (parse_result.source_dirs.size() == 1 ? "directory" : "directories"));
  }
  if (!parse_result.include_dirs.empty()) {
    cforge::logger::print_status(
        "Extracted " + std::to_string(parse_result.include_dirs.size()) + " include "
        + (parse_result.include_dirs.size() == 1 ? "directory" : "directories"));
  }
  ...
  if (!parse_result.compile_definitions.empty()) {
    cforge::logger::print_status(
        "Extracted " + std::to_string(parse_result.compile_definitions.size()) + " compiler "
  ...
  if (backup && std::filesystem::exists(output_fs)) {
    ...
    try {
      std::filesystem::copy_file(output_fs,
                                 backup_path,
                                 std::filesystem::copy_options::overwrite_existing);
      cforge::logger::print_action("Backup", backup_path.string());
    } catch (const std::exception &e) {
      cforge::logger::print_error("Failed to create backup: " + std::string(e.what()));
      return 1;
    }
  }
