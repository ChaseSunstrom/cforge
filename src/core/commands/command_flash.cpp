 ...
 * @brief Implementation of the 'flash' command for uploading firmware to
 * embedded targets
 */
...
cforge_int_t cforge_cmd_flash(const cforge_context_t *ctx) {
  ...
  if (cross_profile.empty()) {
    cforge::logger::print_error("No cross-compilation profile specified. Use --profile <name>");
    cforge::logger::print_plain("  Example: cforge flash --profile avr");
    return 1;
  }
  ...
  if (flash_cmd.empty()) {
    cforge::logger::print_error("Cross profile '" + cross_profile
                                + "' does not have a flash command configured");
    cforge::logger::print_plain("  Add 'flash = \"your-flash-command\"' to [cross.profile."
  ...
  if (config_name.empty()) {
    config_name = project_config.get_string("build.build_type", "Debug");
  }
  ...
  cforge::logger::print_action("Flashing", "using profile '" + cross_profile + "'");

  if (!cforge::run_cmake_build(build_dir, config_name, flash_target, num_jobs, verbose)) {
