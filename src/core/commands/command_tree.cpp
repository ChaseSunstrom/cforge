...
void print_tree_branch(const std::string &name,
  ...
  std::string output = fmt::format(
      "{}{}{}{}", branch, fmt::format(fg(color), "{}", name), version_str, type_indicator);
...
[[maybe_unused]] void collect_dependencies(const fs::path & /*project_dir*/,
    ...
    for (const auto &dep : dep_keys) {
      ...
      if (config.has_key(key + ".version")) {
        ...
        if (config.has_key(key + ".git")) {
          ...
          deps[dep] = info;
        } else if (config.has_key(key + ".vcpkg") || config.get_bool(key + ".vcpkg", false)) {
          // vcpkg dependency
          dependency_info info;
          info.name    = dep;
          info.type    = "vcpkg";
          ...
          deps[dep]    = info;
        } else if (config.has_key(key + ".system") || config.get_bool(key + ".system", false)) {
          // System dependency
          dependency_info info;
          info.name    = dep;
          info.type    = "system";
          info.version = config.get_string(key + ".version", "");
          deps[dep]    = info;
        } else if (config.has_key(key + ".path") || config.has_key(key + ".project")) {
  ...
  if (config.has_key("dependencies.git")) {
    ...
    for (const auto &dep : git_deps) {
      ...
      if (info.version.empty()) {
        info.version = config.get_string("dependencies.git." + dep + ".branch", "");
      }
      if (info.version.empty()) {
        info.version = config.get_string("dependencies.git." + dep + ".commit", "");
        if (!info.version.empty() && info.version.length() > 8) {
 ...
 */
std::vector<project_graph_edge> collect_project_dependencies(const cforge::workspace &ws) {
  static const std::set<std::string> special_keys = {
      "directory", "git", "vcpkg", "system", "project", "fetch_content"};
    ...
    for (const auto &c : conflicts) {
      ...
      for (const auto &o : c.occurrences) {
        cforge::logger::print_plain(
            fmt::format("             {} requires {} ({})", o.project, o.version, o.type));
      }
    }
    cforge::logger::print_plain(fmt::format(
        "  {} dependency conflict{} found", conflicts.size(), conflicts.size() == 1 ? "" : "s"));
  }
}
...
void emit_dot(const std::string &workspace_name,
  ...
  for (const auto &[proj_name, proj_info] : roots) {
    for (const auto &child_name : proj_info.children) {
      ...
      const auto &dep = it->second;
      if (dep.type == "project") {
        continue;  // already covered by inter-project edges
      }
...
cforge_int_t cforge_cmd_tree(const cforge_context_t *ctx) {
  ...
  if (is_workspace) {
    ...
    for (cforge_size_t i = 0; i < roots.size(); i++) {
      ...
      print_tree_branch(
          roots[i].first, roots[i].second, all_deps, "  ", is_last, visited, max_depth, 0);
    }

    // Print summary
    cforge::logger::print_blank();
    cforge_int_t index_count = 0, git_count = 0, vcpkg_count = 0, sys_count = 0, proj_count = 0;
    ...
    std::string project_name = config.get_string("project.name", current_dir.filename().string());
