...
// The builtin test framework header is now written into the build dir by
// test_runner::write_builtin_header(). The user's source tree is no longer
// touched. (Source of truth: tests/test_framework.h; embedded mirror lives in
...
cforge_int_t run_tests_for_project(const std::filesystem::path &project_dir,
  ...
  // The builtin framework header is dropped into the build dir at
  // CMake-generation time by test_runner; nothing to do here.
