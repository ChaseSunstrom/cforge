 ...
 *   1. Reads [hot_reload] from cforge.toml, errors with helpful message if
 * absent.
...
bool launch_host(const std::string &exe_path, const std::string &working_dir) {
#ifdef _WIN32
  STARTUPINFOA si;
...
bool host_is_alive() {
  ...
  DWORD code = 0;
  if (!GetExitCodeProcess(g_host_proc_handle, &code)) {
    return false;
  }
...
struct FileWatcher {
  ...
  FileWatcher(const fs::path &r, const std::vector<std::string> &ext) : root(r), extensions(ext) {
    scan_all();
  }
    ...
    try {
      for (auto &entry : fs::recursive_directory_iterator(root)) {
        ...
        if (ps.find("build") != std::string::npos || ps.find(".git") != std::string::npos
            || ps.find("deps") != std::string::npos || ps.find("vendor") != std::string::npos) {
          continue;
        }
...
bool run_hot_build(const fs::path &project_dir, const std::string &config, bool verbose) {
  auto start = std::chrono::steady_clock::now();

  fs::path build_dir =
      cforge::get_build_dir_for_config((project_dir / DEFAULT_BUILD_DIR).string(), config, true);

  auto prep = cforge::prepare_project_for_build(project_dir,
                                                build_dir,
                                                config,
                                                verbose,
                                                false,  // toml_changed
...
cforge_int_t cforge_cmd_hot(const cforge_context_t *ctx) {
  ...
  if (!reader.has_key("hot_reload.enabled")) {
    ...
    cforge::logger::print_config_block({
        ...
        "watch_dirs  = [\"src\", \"include\"]    # defaults to "
        "[build].source_dirs",
    });
    return 1;
  }
  ...
  std::vector<std::string> hr_watch_dirs = reader.get_string_array("hot_reload.watch_dirs");
  ...
  if (!binary_type.empty() && binary_type != "shared_lib" && binary_type != "shared_library") {
    cforge::logger::print_warning("binary_type = \"" + binary_type
  ...
  fs::path build_dir =
      cforge::get_build_dir_for_config((project_dir / DEFAULT_BUILD_DIR).string(), config, false);

  fs::path host_exe = cforge::find_project_binary(build_dir, project_name, config, "executable");
  if (host_exe.empty() || !fs::exists(host_exe)) {
    ...
    host_exe = cforge::find_project_binary(build_dir, host_stem.string(), config, "executable");
  }
  ...
  if (!host_exe.empty() && fs::exists(host_exe)) {
    ...
    if (!launch_host(host_exe.string(), project_dir.string())) {
      cforge::logger::print_warning("Failed to launch host; continuing in watch-only mode");
    }
  } else {
    cforge::logger::print_warning("Host executable not found; continuing in watch-only mode");
  }
  ...
  std::vector<std::string> watch_extensions = {".cpp", ".cc", ".cxx", ".c", ".hpp", ".hxx", ".h"};
  ...
  while (!g_hot_should_exit) {
    ...
    if (!host_is_alive() && !g_hot_should_exit) {
      ...
      if (g_host_pid != 0) {
#endif
        cforge::logger::print_error("Host process exited unexpectedly; "
    ...
    auto build_ms = std::chrono::duration_cast<std::chrono::milliseconds>(build_end - build_start);

    if (ok) {
      // Write signal file atomically
      ++signal_counter;
      if (!write_signal_file(signal_path, signal_counter)) {
        cforge::logger::print_error("Signal file is unwritable — aborting hot reload session");
        break;
      }

      cforge::logger::print_action("Reloaded",
      ...
      cforge::logger::print_warning("Build failed — kept old " + project_name + " module loaded");
