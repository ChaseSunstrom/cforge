    ...
    for (cforge_int_t i = 2; i < argc; i++) {
        if (strcmp(argv[i], "--config") == 0 && i + 1 < argc) {
            ...
            if (args->args == NULL) {
                ...
                args->args = (cforge_string_t*)malloc((argc - i) * sizeof(cforge_string_t));
                args->arg_count = 0;
            }
...
cforge_int_t cmd_deps(cforge_context_t* ctx) {
    ...
    if (ctx->is_workspace) {
        if (ctx->args.project) {
            cforge_print_status("Installing dependencies for specific project in workspace...");
            // Install dependencies for specific project in workspace
        } else {
            cforge_print_status("Installing dependencies for all projects in workspace...");
...
cforge_int_t cmd_ide(cforge_context_t* ctx) {
    ...
    if (ctx->is_workspace) {
        if (ctx->args.project) {
            cforge_print_status("Generating IDE files for specific project in workspace...");
...
cforge_int_t cmd_list(cforge_context_t* ctx) {
    ...
    if (ctx->is_workspace) {
        ...
        cforge_cstring_t what = (ctx->args.args && ctx->args.arg_count > 0) ? ctx->args.args[0] : "all";
...
cforge_int_t cforge_cmd_help(cforge_context_t* ctx) {
    ...
    printf("\nOptions:\n");
    printf("  --config <name>    Build with specific configuration (Debug, Release)\n");
    ...
    printf("  cforge build                  # Build with default configuration\n");
    printf("  cforge build --config Release # Build with Release configuration\n");
